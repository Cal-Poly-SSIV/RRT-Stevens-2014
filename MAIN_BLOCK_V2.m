...
COUNTERSON=0; %print counter 
 ...
 i = 173;   
%Plan new path for each scan       
%for i = 1:length(Lidar_Data.time(:,1))
   
    
ROAD=RoadBounds.signals.values(:,1,i);
OBSTACLEA=Obstacles.signals.values(:,:,i);
...
%PHYSICAL OBJECTS**********************
VEHICLE=[0,0,0,1];      %INITIAL STATE [x,y,theta,v]
...
%OBSTACLEA = Obstacles; % added by Ian %
%Trimming Function
tic
...
% plot(Lidar_Data.signals.values(:,1,i),Lidar_Data.signals.values(:,2,i));
