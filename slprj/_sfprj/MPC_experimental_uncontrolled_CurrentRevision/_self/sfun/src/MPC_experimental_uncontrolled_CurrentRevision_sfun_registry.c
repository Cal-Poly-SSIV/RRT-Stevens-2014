#include "MPC_experimental_uncontrolled_CurrentRevision_sfun.h"
#include "sfcdebug.h"
...
{
  extern unsigned int
    sf_MPC_experimental_uncontrolled_CurrentRevision_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_MPC_experimental_uncontrolled_CurrentRevision_process_check_sum_call
      (nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_MPC_experimental_uncontrolled_CurrentRevision_autoinheritance_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_MPC_experimental_uncontrolled_CurrentRevision_autoinheritance_info(nlhs,
       plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_MPC_experimental_uncontrolled_CurrentRevision_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "MPC_experimental_uncontrolled_CurrentRevision") == 0)
  {
...
extern unsigned int
  sf_MPC_experimental_uncontrolled_CurrentRevision_method_dispatcher(SimStruct
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"MPC_experimental_uncontrolled_CurrentRevision")) {
    return(sf_MPC_experimental_uncontrolled_CurrentRevision_method_dispatcher
...
extern void MPC_experimental_uncontrolled_CurrentRevision_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    MPC_experimental_uncontrolled_CurrentRevision_terminator();
    sf_debug_terminate();
  }
...
extern void MPC_experimental_uncontrolled_CurrentRevision_initializer(void);
extern void
  MPC_experimental_uncontrolled_CurrentRevision_register_exported_symbols
  (SimStruct* S);
extern void MPC_experimental_uncontrolled_CurrentRevision_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  MPC_experimental_uncontrolled_CurrentRevision_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      MPC_experimental_uncontrolled_CurrentRevision_debug_initialize();
    }

    MPC_experimental_uncontrolled_CurrentRevision_initializer();
  }
