function mpcblockiotable(mpcblockname, model, linsetting)
...
%   $Revision: 1.1.6.14 $  $Date: 2009/11/09 16:28:22 $
...
mosrcblk = getfullname(portconnection(1).SrcBlock);

%% Get the MD port width and extract the source for the MD port
[nmd, mdsrcblk] = mpc_getnmd(mpcblockname);
...
for k=1:length(mosrcblk_portconn)
    ...
    if ~isempty(ind) && any(mosrcblk_portconn(k).DstPort(ind)==0)
...
if nmd>0
    ...
    for k=1:length(mdsrcblk_portconn)
        ...
        if ~isempty(ind) && any(mdsrcblk_portconn(k).DstPort(ind)==2)
