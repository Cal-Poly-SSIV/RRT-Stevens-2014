...
%   $Revision: 1.1.8.8 $  $Date: 2007/11/09 20:46:41 $
...
if ptype>=1,
    ...
    if condnum>bigcondnum,
        warning('MPC:computation:HessianSingular',ctrlMsgUtils.message('MPC:computation:HessianSingular'));
...
if any(dumin(index2,:)>0),
    if verbose,
        fprintf('-->%s',ctrlMsgUtils.message('MPC:computation:PosLowerBound'));
    end
end
if any(dumax(index2,:)<0),
    if verbose,
        fprintf('-->%s',ctrlMsgUtils.message('MPC:computation:NegUpperBound'));
    end
end
dumin(index2,:)=[];
dumax(index2,:)=[];
...
% Remove constraints also in Mu,Mx,Mu1,Mv,Md
...
% NOTE: The assumption made in MPC_COMMON.C about the structure of MLIM
% must be consistent with what is defined here below!
