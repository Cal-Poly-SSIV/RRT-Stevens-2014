...
%   $Revision: 1.1.10.9 $  $Date: 2008/07/14 17:09:50 $


% deal with empty mpc object
if isempty(MPCobj)
...
fprintf('Sampling time:      %s',num2str(MPCobj.Ts));
fprintf('Prediction Horizon: %d',p);
fprintf('Control Horizon:    %s\n',num2str(m));
...
uoff=zeros(nu,1);
try
    [umin,umax,Vumin,Vumax,dumin,dumax]=...
...
yoff=zeros(ny,1);
try
    [ymin,ymax]=mpc_constraints(MPCobj.OutputVariables,...
        p,ny,'y',yoff);
catch ME
    throw(ME);
end
