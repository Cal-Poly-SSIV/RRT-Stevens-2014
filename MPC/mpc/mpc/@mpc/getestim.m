...
%   $Revision: 1.1.8.6 $  $Date: 2007/11/09 20:39:16 $

if isempty(MPCobj),
    ctrlMsgUtils.error('MPC:general:EmptyMPCObject','getestim');
end

if nargin<2 || isempty(ssoutput),
    ssoutput='mat';
else
    if ~ischar(ssoutput) || (~strcmpi(ssoutput,'sys') && ~strcmpi(ssoutput,'mat')),
        ctrlMsgUtils.error('MPC:estimation:InvalidModelType','getestim');
    else
        ssoutput=lower(ssoutput);
    end
end
