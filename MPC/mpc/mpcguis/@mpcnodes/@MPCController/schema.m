...
%  $Revision: 1.1.8.10 $ $Date: 2007/11/09 20:42:43 $
...
% The following holds the estimator refresh status flag.  Must be set
% to true to allow an estimator refresh in RefreshEstimStates.
schema.prop(c,'EstimRefreshOK','int32');
...
if ~isempty(MPCControllers)
    ...
    if ~isempty(Controllers.find('Label', eventData))
        ...
        msg = ctrlMsgUtils.message('MPC:designtool:DuplicatedControllerName', eventData, propval);
