...
%  $Revision: 1.1.8.6 $ $Date: 2008/06/20 08:03:12 $
...
for i=1:Nvar
    ...
    if ~isempty(ix)
        switch Data{i,3}
            ...
            case 'Pulse'
                ix = (t >= t0(i)) & (t < t0(i) + Period(i));
                x(ix,i) = x0(i) + Mag(i);
            case 'Gaussian'
                % Draw values from a private random number stream
                % initialized with a near-random starting point
                s = RandStream('shr3cong','seed',sum(100*clock));
                x(ix,i) = Mag(i)*randn(s,size(ix(:))) + x0(i);
%             otherwise
