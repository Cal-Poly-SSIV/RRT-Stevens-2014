...
%  $Revision: 1.1.8.13 $ $Date: 2007/11/09 20:44:57 $
...
S = this.getChildren;
if isempty(S) 
    disp('MPCSims:  No scenario nodes defined.')
    return
end
...
if isfield(S.Handles,'SimulateMenu')  % Might not be created yet
    awtinvoke(S.Handles.SimulateMenu.getAction,'setEnabled(Z)',true);
end
...
[DialogPanel, Buttons, NotesArea] = ComponentListPanel( Width, ...
    UTitle, LTitle, NTitle, t, Labels, Labels, LContent, 'MPCSims');
...
MPCCombo = MJComboBox;
MPCCombo.setEditable(false);
PlantCombo = MJComboBox;
PlantCombo.setEditable(false);

% Callbacks
set(handle(Buttons.New, 'callbackproperties'), ...
    'ActionPerformedCallback', {@LocalAddButtonCB, this});
set(handle(Buttons.Copy, 'callbackproperties'), ...
    'ActionPerformedCallback', {@LocalCopyButtonCB, this});
...
set(handle(Buttons.Help, 'callbackproperties'), ...
    'ActionPerformedCallback', {@mpcCSHelp, 'MPCSIMSMAIN'});
...
elseif col == 6
    ...
    if ~isnan(Num) && Num > 0 && Num < Inf
        ...
        msg = ctrlMsgUtils.message('MPC:designtool:InvalidDurationValue');
        uiwait(errordlg(msg, ctrlMsgUtils.message('MPC:designtool:DialogTitleError'), 'modal'));
        OK = false;
    end
% else
%     error('Unexpected table column "%i"', col);
end
if OK
    Node.HasUpdated = 1;
    Node.setNewDate;
