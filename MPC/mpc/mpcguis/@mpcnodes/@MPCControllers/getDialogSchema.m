...
%  $Revision: 1.1.8.13 $ $Date: 2007/11/09 20:43:00 $
...
C = this.getChildren;
if isempty(C) 
    disp('No controller nodes defined under MPCControllers.')
    return
end
...
Labels = {'Import ...', 'Export ...', 'New', 'Copy', 'Display', ...
    'Delete', 'Help'};
Fields = Labels;
Fields(1:2) = {'Import', 'Export'};
[DialogPanel, Buttons, NotesArea] = ComponentListPanel( Width, ...
    UTitle, LTitle, NTitle, t, Labels, Fields, LContent, 'MPCControllers');
...
if col == 1
    ...
    if ~strcmp(ControllerName, String) && ~isempty(deblank(String))
...
elseif col == 3
    ...
    if isnan(Value) || Value <= 0 || Value == Inf
        msg = ctrlMsgUtils.message('MPC:designtool:InvalidModelHorizonTabIntervalError');
...
elseif col == 4
    ...
    if ~isnan(Num) && Num > 0 && Num < Inf && abs(Num - round(Num)) < eps
        ...
        msg = ctrlMsgUtils.message('MPC:designtool:InvalidModelHorizonTabHorizonError');
        uiwait(errordlg(msg, ctrlMsgUtils.message('MPC:designtool:DialogTitleError'), 'modal'));
        OK = false;
    end
% else
