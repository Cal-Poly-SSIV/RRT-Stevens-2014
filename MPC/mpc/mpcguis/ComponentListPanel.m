function [Panel, Buttons, NotesArea] = ComponentListPanel( Width, ...
    UTitle, LTitle, NTitle, UContent, ButtonLabels, ButtonFields, ...
    LContent, DialogName)
% Creates panel to hold list of models, controllers, or scenarios

%	Author:  Larry Ricker
%	Copyright 1986-2007 The MathWorks, Inc.
%	$Revision: 1.1.8.4 $  $Date: 2007/11/09 20:42:04 $
...
UContent.Table.setName([DialogName, '_List']);
...
for i = 1:length(ButtonLabels)
    ...
    thisButton.setName([DialogName, '_', ButtonFields{i}]);
    FieldName = ButtonFields{i};
    k = strfind(FieldName, ' ');
    if ~isempty(k)
        FieldName = FieldName(1:k(1)-1);
    end
    Buttons.(FieldName) = thisButton;
...
NotesArea.setName([DialogName, '_Notes']);
